...
function ColorEditor({ title, color, setColor }) {
  ...
  // metadata fields for this color
  ...
  return (
    <div className="grid grid-cols-[auto,1fr] gap-x-4 gap-y-2 items-center">
      ...
      <div className="row-span-6 flex items-center"><VBar color={rgbToHex(color)} /></div>
      ...
      <div className={`flex flex-wrap items-center gap-2 ${textClass}`}>
        <input type="number" min={0} max={100} step={0.1} value={Number(lab.L.toFixed(1))}
          onChange={(e)=>{ const L=parseFloat(e.target.value); if(!Number.isNaN(L)) setColor(labToRgb({L, a:lab.a, b:lab.b})); }}
          className={`w-20 rounded-md border px-2 py-1 ${inputClass}`} placeholder="L"/>
        <input type="number" min={-128} max={128} step={0.1} value={Number(lab.a.toFixed(1))}
          onChange={(e)=>{ const a=parseFloat(e.target.value); if(!Number.isNaN(a)) setColor(labToRgb({L:lab.L, a, b:lab.b})); }}
          className={`w-20 rounded-md border px-2 py-1 ${inputClass}`} placeholder="a"/>
        <input type="number" min={-128} max={128} step={0.1} value={Number(lab.b.toFixed(1))}
          onChange={(e)=>{ const b=parseFloat(e.target.value); if(!Number.isNaN(b)) setColor(labToRgb({L:lab.L, a:lab.a, b})); }}
          className={`w-20 rounded-md border px-2 py-1 ${inputClass}`} placeholder="b"/>
      </div>
    </div>
  );
}
