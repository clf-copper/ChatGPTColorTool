...
function xyzToRgb({ X, Y, Z }) {
  ...
  const to8 = (v) =>
    clamp(Math.round(linearToSrgb(Math.max(0, Math.min(1, v))) * 255));
...
const mixRgb2 = (A, B, tB) => {
  ...
  const ch = (ca, cb) =>
    clamp(
      Math.round(fromLin(mixLinear(srgbToLinear(ca), srgbToLinear(cb), tB)))
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm items-center">
        ...
        <div className={`font-mono ${textClass}`}>{resultHex}</div>
        <div className={`font-semibold ${labelClass}`}>RGB</div>
        <div
          className={`font-mono ${textClass}`}
        >{`${result.r}, ${result.g}, ${result.b}`}</div>
        <div className={`font-semibold ${labelClass}`}>LRV</div>
        <div className={`font-mono ${textClass}`}>{lrv(result).toFixed(1)}</div>
        <div className={`font-semibold ${labelClass}`}>CIELAB</div>
        <div
          className={`font-mono ${textClass}`}
        >{`${lab.L.toFixed(1)}, ${lab.a.toFixed(1)}, ${lab.b.toFixed(1)}`}</div>
  ...
  return (
    <div className="space-y-4">
      ...
      <input
        ...
        min={0}
        max={100}
        value={Math.round(tB * 100)}
        onChange={(e) => setTB(Number(e.target.value) / 100)}
        className="w-full"
      />
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-xs opacity-70">
        Click/drag inside the circle. Pointer marks selection and updates the
        result.
      </div>
    </div>
  );
  ...
  const handleSolve = () => {
    ...
    if (pantry && pantry.length > 0) {
      ...
      if (closest) {
        ...
        setBanner({
          kind: "warn",
          text: "No pantry paints available to compare.",
        });
      }
    }
  };
    ...
    <div className="p-4 border rounded space-y-3 bg-gray-50">
      ...
      <div className="flex gap-2 text-sm">
        <input
          ...
          value={targetLab.L}
          onChange={(e) =>
            setTargetLab({ ...targetLab, L: parseFloat(e.target.value) })
          }
        ...
        <input
          ...
          value={targetLab.a}
          onChange={(e) =>
            setTargetLab({ ...targetLab, a: parseFloat(e.target.value) })
          }
          className="border rounded px-2 py-1 w-20"
          placeholder="a"
        />
        <input
          ...
          value={targetLab.b}
          onChange={(e) =>
            setTargetLab({ ...targetLab, b: parseFloat(e.target.value) })
          }
          className="border rounded px-2 py-1 w-20"
          placeholder="b"
        />
